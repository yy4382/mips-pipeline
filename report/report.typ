...
本次实验实现了一个简单的指令解析器，不支持汇编指示符 (`.text`, `.data` 等)，但支持除了无符号数运算之外的大部分普通指令，包括：
...
出于方便考虑，我简化了指令内存和数据内存的访问方式。将一个寄存器的长度记为一个单位；iMem 和 dMem 都实现为每个元素常为单位的数组。
...
这样的简化主要是因为 JavaScript 中所有数字都是 IEEE 754 双精度浮点数，无法直接表示 32 位整数，这样将一个双精度浮点数是当作一个“单位”对于「模拟流水线」的目标来说差异并不太大，但是在实现上会简单很多。
...
在流水线模拟器中，尽量模仿了实际 CPU 的设计，比如在流水线中，使用了指令译码而来的控制信号来控制每个阶段的运行，而不是在每个阶段里对各种指令类型 if/else 来处理。在开发方面，也是为了方便重构，添加了大量测试，最终在模拟器模块中测试的覆盖率达到了 84%。（运行 `pnpm vitest --coverage`）

在前端方面，我设计了一个直观易用的展示界面，同时得益于 Web 生态，美观程度也好于其他大部分模拟器。同时，网页的另一个好处是可以随处直接运行，不需要下载或编译，打开浏览器即可直接运行。

== 测试代码分析

测试 1 是网页打开后的默认指令；测试 2、测试 3 和测试 5 的代码可以在网页上指令输入框下方的快速选择中直接载入。
...
可以发现，PC 和 IF/ID 流水线寄存器保持不变，而 ID/EX 流水线寄存器中的指令被替换成了 NOP，从而实现了停顿。
...
本例中定义 Fib(0) 无意义，Fib(1) = 0，Fib(2) = 1.
